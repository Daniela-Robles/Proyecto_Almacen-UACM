    ...
    <>
      ...
      <aside className="sidebar">
        ...
        <nav className="sidebar-nav">
          ...
          {modulos.map((m, i) => (
            <a key={i} href={m.url}
              className={`sidebar-link${m.url === '/Solicitudes/' ? ' sidebar-link--active' : ''}`}>
      ...
      <div className="main-wrapper">
        ...
        <div className="topbar">
          <div>
            <p className="topbar-title">Solicitud de Artículos</p>
            <p className="topbar-subtitle">Registro, consulta y control de solicitudes</p>
          </div>
          <div className="topbar-actions">
            {datos.user_role?.toLowerCase().includes('encargado') && (
              <button type="button" className="topbar-btn" onClick={abrirModalLimites} title="Gestionar límites de solicitud">
                <i className="fas fa-sliders-h"></i><span>Límites</span>
              </button>
            )}
            {campanaVisible && (
              <button type="button" className="topbar-bell" onClick={() => setModalAlertas(o => !o)} title={`${alertasStock.length} producto(s) con stock bajo`}>
                <i className="fas fa-bell"></i>
                <span className="topbar-bell-badge">{alertasStock.length}</span>
              </button>
            )}
          </div>
        </div>
        ...
        <main className="main-content">
          <div className="content-wrapper">
            ...
            {/* Buscar solicitud */}
            <section className="card card--buscar buscar-section">
            ...
            {/* Nueva solicitud */}
            <section className="card card--nueva" id="nueva-solicitud-container">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingBottom: '.75rem', borderBottom: '2px solid rgba(100,4,4,.10)', marginBottom: '1.5rem' }}>
